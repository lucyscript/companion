...
| ⬜ todo | `subscription-plans-billing` | fullstack-engineer | Implement tiered subscription plans (free trial → paid) with Stripe integration for in-app payment. Free tier: limited Gemini calls/day. Paid tiers: higher rate limits, all integrations, priority coaching. Requires Stripe account + webhook setup. |
| ⬜ todo | `per-user-rate-limiting` | backend-engineer | Add per-user Gemini API rate limiting and quota tracking so free-tier users share a capped pool while paid users get dedicated quotas. Track usage in storage with daily reset. |
