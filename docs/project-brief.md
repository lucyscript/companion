...
| ✅ done | `nutrition-meal-plan-reference-cleanup` | backend-engineer | Remove deprecated meal-plan references from Gemini nutrition intent/tool surface and document `/api/nutrition/plan*` endpoints as legacy compatibility only so active guidance stays on item-based meal logging. Tracked in issue #459. |
