...
| ✅ done | `chat-journal-direct-save` | backend-engineer | Save journal entries immediately from chat tool calls without confirm/cancel action commands. |
...
| ✅ done | `nutrition-ui-meal-plan-first` | frontend-engineer | Rework Nutrition tab into meal-plan-first flow with top-level plan settings (weight, maintenance/surplus, macro target overrides), derived target preview, and daily totals with target deltas. Tracked in issue #361. |
...
| ✅ done | `env-prefix-cleanup` | backend-engineer | Use canonical non-prefixed server env names (`TIMEZONE`, `USER_NAME`, `VAPID_*`, provider vars). Legacy `AXIS_*` aliases fully removed. |
...
| ✅ done | `github-dat520-lab-plan-deadlines` | backend-engineer | Extend GitHub course sync to ingest DAT520 `info` lab-plan deadlines (`Lab | Topic | Deadline` docsify markdown), normalize month/day dates to current-year ISO, and map lab rows into assignment deadlines visible in Deadlines tab. |
| ✅ done | `github-deadline-file-scan` | backend-engineer | Extend GitHub deadline sync beyond README by scanning deadline-bearing markdown files (e.g. `lab-plan.md`) and parsing/deduping assignment deadlines from those sources. |
| ✅ done | `github-deadline-hint-fallback` | backend-engineer | Add per-repo fallback deadline file hints (like `lab-plan.md`) so deadline ingestion still works when GitHub tree listing endpoints are unavailable with current PAT scopes. |
...
| ✅ done | `github-course-citations-in-chat` | backend-engineer | Add tool/citation support so chat responses can reference specific synced GitHub syllabus/course docs (path + snippet) instead of only implicit context injection. |
| ✅ done | `ios-keyboard-safearea-hardening` | frontend-engineer | Harden chat/journal input layouts for iPhone Safari keyboard + safe-area edge cases (rotation, PWA standalone, notch devices) with zero clipped controls. |
| ✅ done | `sync-auto-healing-policies` | backend-engineer | Add per-integration auto-healing rules (backoff windows, temporary circuit-breakers, jittered retries) to reduce repeated failure storms and improve reliability without manual intervention. |
| ✅ done | `schedule-day-timeline-ui` | frontend-engineer | Add a compact day-timeline view (lecture blocks, free gaps, upcoming deadline markers) optimized for one-glance planning on iPhone. |
| ✅ done | `schedule-full-day-allocation` | fullstack-engineer | Rename Lecture Schedule to Schedule, allocate non-lecture time into suggested planning blocks in the day timeline, and enable chat-driven schedule edits via `queueUpdateScheduleBlock` confirmation flow. Tracked in issue #333. |
| ✅ done | `schedule-primary-view-no-study-plan` | frontend-engineer | Remove Study Plan toggle from Schedule tab so Schedule + Deadlines is the primary planning surface, while habits/goals remain the check-in loop. Tracked in issue #345. |
| ✅ done | `ui-24h-digital-time-format` | frontend-engineer | Standardize UI time rendering to 24-hour digital format (`HH:mm`, no AM/PM) across chat, schedule, deadlines, notifications, and import/preview surfaces. Tracked in issue #345. |
| ✅ done | `analytics-gemini-coach` | fullstack-engineer | Replace chart-first analytics with Gemini narrative coaching over selectable windows (7/14/30 days), with strengths, risks, and steering recommendations. |
...
| ✅ done | `nutrition-custom-foods-library` | fullstack-engineer | Add macro-only custom foods library with CRUD endpoints (`GET/POST/PATCH/DELETE /api/nutrition/custom-foods`), Nutrition UI create/edit/delete/reuse flow, and Gemini tools for creating/updating/deleting custom foods plus meal logging from custom-food templates. Tracked in issue #360. |
...
| ✅ done | `ios-chat-focus-scroll-fix` | frontend-engineer | Remove chat input focus auto-scroll behavior (`scrollIntoView`) that caused iPhone keyboard interactions to push the full chat container upward while typing. Tracked in issue #369. |
| ✅ done | `schedule-widget-today-room-focus` | fullstack-engineer | Update Schedule widget to show today-only lecture entries, render room labels from lecture locations (e.g. `KE-101`), and remove `Suggested:` prefix from Gemini-planned timeline block titles. Tracked in issue #378. |
| ✅ done | `habit-goal-intent-autocapture` | fullstack-engineer | Add intent-level habit/goal autocapture in chat: detect repeated commitment language ("I keep missing morning gym", "I want to read nightly"), propose create/update actions, and generate one-tap confirmation prompts with rationale. Tracked in issue #355. |
...
| ✅ done | `schedule-timeline-upcoming-deadlines-removal` | frontend-engineer | Remove duplicate “Upcoming deadlines” subsection from the Today timeline in Schedule since deadlines are already visible in the dedicated deadlines surface. Tracked in issue #450. |
| ✅ done | `gemini-deadline-retrieval-course-filters` | backend-engineer | Improve `getDeadlines` tool reliability by adding course/query filtering, broader default horizon, sorted results, and richer deadline payload compaction so DAT520/DAT560 deadline queries resolve consistently in chat. Tracked in issue #447. |
| ✅ done | `nutrition-meal-plan-workflow-removal` | frontend-engineer | Remove meal-plan block creation/reorder/quick-add workflow from Nutrition and keep meal logging + custom-food flow as the primary interaction model. Tracked in issue #452. |
| ✅ done | `nutrition-meal-items-parity` | fullstack-engineer | Rework nutrition meal logging so each meal is composed of food items (`name/quantity/unit/per-unit macros`) and derive per-meal macro totals from item composition in API/storage/UI. Tracked in issue #449. |
| ✅ done | `nutrition-layout-target-derivation-parity` | frontend-engineer | Move Nutrition daily totals to the bottom of the tab and align target derivation with `lucyscript/food` behavior (calories from maintenance+surplus, protein/fat bodyweight-derived with range guidance, carbs auto-filled from remaining calories). Tracked in issue #451. |
| ✅ done | `nutrition-meal-plan-reference-cleanup` | backend-engineer | Remove deprecated meal-plan references from Gemini nutrition intent/tool surface so active guidance stays on item-based meal logging. Tracked in issue #459. |
...
| ⬜ todo | `ad-supported-free-tier` | fullstack-engineer | Add non-intrusive ad placements for free-tier users: banner ads between schedule blocks, interstitial after every N chat messages, and a small banner in Settings. Paid users see no ads. Use Google AdMob or similar PWA-compatible ad SDK. |
| ⬜ todo | `per-user-rate-limiting` | backend-engineer | Add per-user rate limiting across all API endpoints (not just Gemini) so free-tier users share a capped pool while paid users get dedicated quotas. Track usage in storage with daily reset. |
